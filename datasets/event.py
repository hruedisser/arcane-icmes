...
class Event:
    ...
    def __init__(self, begin, end, mobegin=None, param=None, origin = None):
        self.begin = begin
...
def clearempties(evtlist, data):
    """
    Clear events from list for which no data exists.
    """
    evtlistnew = []
...
def cleardata(evtlist,data):
    
    """
    Remove data from dataset for which no catalog exists.
    """
    ...
    for year in range(startyear,endyear+1):
        ...
        if len([i for i in evtlist if (i.begin.year == year)]) != 0:
            ...
            if i == 0:
                ...
                i = i+1
                logger.info('data for %i added' %year)
            else:
                cleaneddata = cleaneddata.append(data[data.index.year == year])
                logger.info('data for %i added' %year)
    
    return cleaneddata, eventyears
...
def similarity(event1, event2):
    """
    Return the similarity of twi events.
    """
    if event1 is None:
        return 0
...
def read_cat(begin, mobegin, end, iwinind, dateFormat="%Y/%m/%d %H:%M",
             sep=','):
    ...
    mobegin = pds.to_datetime(mobegin, format=dateFormat)
    end = pds.to_datetime(end, format=dateFormat)
    for i in iwinind:
        evtList.append(Event(begin[i], end[i], mobegin[i]))
    return evtList
...
#    try:
    ax1.set_ylim(-np.nanmax(data['bt'])-5,np.nanmax(data['B'])+5)
...
#    plt.setp(ax1.get_xticklabels(), visible=False)
    ...
    plt.title(typ+'ICME, start: '+start.strftime("%Y-%b-%d %H:%M")+'  end: '+end.strftime("%Y-%b-%d %H:%M"))
...
def plot_insitu_icmecat_mag_plasma(data, start, end, delta, i, typ, predstart, predend):
     ...
     ax1.plot_date(data.index, data['bt'],'-r',label='Bx',linewidth=0.5)
     ax1.plot_date(data.index, data['bt'],'-g',label='By',linewidth=0.5)
     ax1.plot_date(data.index, data['bt'],'-b',label='Bz',linewidth=0.5)
     ax1.plot_date(data.index, data['bt'],'-k',label='Btotal',lw=0.5)
...
def get_catevents(catdir,spacecraft):
    
    # load ICME catalog data
    [ic,header,parameters] = pickle.load(open(catdir, "rb" ))
    # extract important values
    isc = ic.loc[:,'sc_insitu'] 
    starttime = ic.loc[:,'icme_start_time']
    endtime = ic.loc[:,'mo_end_time']
    mobegintime = ic.loc[:, 'mo_start_time']

    # Event indices

    ind = np.where(isc == spacecraft)[0]

    begin = starttime[ind]
    mobegin = mobegintime[ind]
    end = endtime[ind]

    evtList = read_cat(begin, mobegin, end, ind)
    
    return evtList   

def read_chinchilla(catdir):
    dateFormat="%Y %m/%d %H:%M"
    df = pds.read_csv(catdir + 'Nieves_Chinchilla.csv')
...
def read_chi(catdir):
    ...
    df = pds.read_csv(catdir+'Chi.csv')
...
def read_lepping(catdir):
    df = pds.read_csv(catdir + 'lepping.csv', dtype=str)
...
def read_richardson(catdir):
    df = pds.read_csv(catdir+'richardson.csv')
    dateFormat="%Y/%m/%d %H%M"
...
def getallcat(config):
    helcats = get_catevents(config['data_dir'] + 'HELIO4CAST_ICMECAT_v21_pandas.p',config['spacecraft'])
    nguyen = read_csv(config['data_dir'] +'listOfICMEs.csv', index_col=None)
    chinchilla = read_chinchilla(config['data_dir'])
    richardson = read_richardson(config['data_dir'])
    chi = read_chi(config['data_dir'])
    lepping = read_lepping(config['data_dir'])
    
    catdic = {'helcats': helcats, 'nguyen': nguyen, 'chinchilla': chinchilla, 'richardson':richardson,'chi':chi,'lepping':lepping}
